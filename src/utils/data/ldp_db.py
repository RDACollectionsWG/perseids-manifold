...
class LDPDataBase(DBInterface):
    ...
    def get_collection(self, id=None):
        ...
        if id is not None:
            result = JSONResult(requests.post(self.marmotta.sparql.select, data=self.sparql.collections.select(self.marmotta.ldp(encoder.encode(id))), headers={"Accept":"application/sparql-results+json", "Content-Type":"application/sparql-select"}).json())
            ...
            for collection in collections:
                result = JSONResult(requests.post(self.marmotta.sparql.select, data=self.sparql.collections.select(collection), headers={"Accept":"application/sparql-results+json","Content-Type":"application/sparql-select"}).json())
    ...
    def get_member(self, cid, mid=None):
        ...
        if mid is not None:
            ...
            for member in members:
                result = JSONResult(requests.post(self.marmotta.sparql.select, data=self.sparql.members.select(member), headers={"Accept":"application/sparql-results+json","Content-Type":"application/sparql-select"}).json())
    ...
    def set_member(self, cid, m_obj):
        ...
        found = JSONResult(requests.post(self.marmotta.sparql.select, data=self.sparql.collections.ask(c_id), headers={"Accept":"application/sparql-results+json","Content-Type":"application/sparql-select"}).json()).askAnswer
        if not found:
            raise KeyError
        ds = Dataset()
    ...
    def del_member(self, cid, mid):
        id = self.marmotta.ldp(encoder.encode(cid)+"/member/"+encoder.encode(mid))
        found = JSONResult(requests.post(self.marmotta.sparql.select, data=self.sparql.members.ask(id), headers={"Accept":"application/sparql-results+json","Content-Type":"application/sparql-select"}).json()).askAnswer
        if found:
    ...
    def get_service(self):
        ...
        found = JSONResult(requests.post(self.marmotta.sparql.select, data=self.sparql.service.ask(id), headers={"Accept":"application/sparql-results+json", "Content-Type":"application/sparql-select"}).json()).askAnswer
        if found:
